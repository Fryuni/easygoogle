...
def test_creation_call_server(mocker, tmpdir):
    ...
    easygoogle.InstalledAppFlow.from_client_secrets_file.return_value = easygoogle.InstalledAppFlow()
    easygoogle.InstalledAppFlow().run_local_server.return_value = credentials_mock

    oauth = easygoogle.oauth2(mocker.sentinel.json_file, ['scope.unique'],
                              user="testuser", app_dir=str(tmpdir.dirpath()))

    easygoogle.InstalledAppFlow.from_client_secrets_file.assert_called_once_with(mocker.sentinel.json_file,
                                                                                 scopes=["https://testscopes.exemple.org/auth/scope.unique"])

    open_mock.assert_called_once_with(str(tmpdir.dirpath('.credentials', 'google_client_library_-_python#testuser.json')), 'w')
    easygoogle.json.dump.assert_called_once_with(model_spec, open_mock())

    credentials_mock.refresh.assert_called_once_with(easygoogle.google.auth.transport.requests.Request())
